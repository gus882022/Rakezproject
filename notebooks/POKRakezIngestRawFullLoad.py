...
display(list_logs_tables)

# COMMAND ----------

df.select('*',current_date().alias('ts_load')).show()

# COMMAND ----------

# MAGIC %sql
# MAGIC drop table  sales_stores

# COMMAND ----------

# MAGIC %sql
# MAGIC select cast(ts_load as varchar(50)) from raw_data_rakez.production_brands

# COMMAND ----------

# MAGIC %sql
# MAGIC use raw_data_rakez
...
df = df.select('*',current_date().alias('ts_load'))
save_delta_table(df,path_delta_table,database_raw,f"{schema}_{table_name}")
regs_written= spark.sql(f"select count(*) from {database_raw}.{schema}_{table_name} where cast(ts_load as varchar)={str(today)}")
value={'Result': 'Table Sucessfull Loaded','regs_loaded': regs_loaded,'regs_written':regs_written}
list_logs_tables[f"{schema}_{table_name}"] = 'Table Sucessfull Loaded'
